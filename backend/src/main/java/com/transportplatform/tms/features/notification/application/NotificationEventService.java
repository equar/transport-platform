...
public class NotificationEventService {
        ...
        public void publishCompanyApplicationSubmitted(CompanyApplication application) {
                notifyTenantAdmins(
                                ...
                                "Company application " + application.getApplicationNumber()
        ...
        public void publishCompanyApplicationApproved(CompanyApplication application) {
                ...
                appUserRepository.findByIdAndTenantId(application.getOwnerUserId(), application.getApprovedTenantId())
                                .ifPresent(owner -> notifyUser(
                                                ...
                                                "Your company application " + application.getApplicationNumber()
                                                                + " was approved.",
                                                "Company application approved",
                                                Map.of(
                                                                "applicationNumber", application.getApplicationNumber(),
        ...
        public void publishCompanyApplicationRejected(CompanyApplication application) {
                ...
                String body = "Company application " + application.getApplicationNumber() + " was rejected."
                                + (application.getRejectionReason() == null
                                                || application.getRejectionReason().isBlank()
                                                                ? ""
        ...
        public void publishCompanyUserCreated(AppUser user) {
                notifyUser(
                                ...
                                Map.of("email", user.getEmail(), "roles",
        ...
        public void publishPasswordResetRequested(AppUser user, String resetUrl) {
                if (user.getEmail() == null || user.getEmail().isBlank() || resetUrl == null || resetUrl.isBlank()) {
                        return;
                }

                String subject = "Password reset instructions";
                String body = "A password reset was requested for your workspace account. Use this link to choose a new password: "
                                + resetUrl;
                notificationEmailSender.send(new NotificationEmailSender.NotificationEmailCommand(
                                user.getEmail().trim(),
                                subject,
        ...
        public InvitationDeliveryResult publishUserInvitation(AppUser user, String activationUrl) {
                if (user == null || user.getEmail() == null || user.getEmail().isBlank() || activationUrl == null
                                || activationUrl.isBlank()) {
                        return new InvitationDeliveryResult(
                                        NotificationDeliveryStatus.SKIPPED,
                                        "Invitation delivery was skipped because the recipient email or activation link was unavailable.");
                }

                String subject = "Set up your workspace account";
                String title = "Complete your account setup";
                String body = "Your workspace account is ready. Use this secure link to choose your password and activate access: "
                                + activationUrl;
                ...
                if (user.getTenantId() == null || user.getTenantId().isBlank() || user.getId() == null) {
                        NotificationEmailSender.DeliveryResult result = notificationEmailSender.send(
                                        new NotificationEmailSender.NotificationEmailCommand(
                                                        ...
                                                        body));
                        return new InvitationDeliveryResult(
                                        result.sent() ? NotificationDeliveryStatus.SENT
                                                        : NotificationDeliveryStatus.FAILED,
                                        result.errorMessage());
                }

                var notification = notificationDispatchService.notifyEmail(
                                user.getTenantId(),
                                user.getId(),
                                user.getEmail(),
                                ...
                                body,
                                context);
                return new InvitationDeliveryResult(notification.getDeliveryStatus(), notification.getErrorMessage());
        }
        ...
        public void publishInvoiceIssued(Invoice invoice) {
                notifyTenantAdmins(
                                ...
                                invoice.getId(),
                                "Invoice issued",
                                "Invoice " + invoice.getInvoiceNumber() + " was issued for "
        ...
        public void publishComplianceIssueOpened(ComplianceIssue issue) {
                notifyTenantAdmins(
                                ...
                                Map.of(
                                                ...
                                                "relatedDocumentType",
                                                issue.getRelatedDocumentType() == null ? ""
        ...
        private void notifyTenantAdmins(String tenantId,
                ...
                for (AppUser adminRecipient : adminRecipients) {
                        notifyUser(adminRecipient, notificationType, relatedEntityType, relatedEntityId, title, message,
                                        title,
                                        context);
                }
        }
